      ...
      <div class="col col-md-8 col-lg-6">
        <div class="row">
          <div class="col-auto mb-4">
            <a href="javascript:void(0)" @click.prevent="goBack"
              ><span class="icon"
                ><font-awesome-icon class="fa-xs" icon="arrow-left" /></span
              ><span v-if="display_back_url">{{
                t("views.database.backButton")
              }}</span>
              <span v-else>{{ t("views.database.homeButton") }}</span></a
            >
          </div>
        </div>
        ...
        <div v-if="database.malfunction_message_active" class="row mb-4">
          <div class="col">
            <div class="alert alert-warning" role="alert">
              ...
              />
              <span
                class="malfunction-message"
                v-html="malfunction_message_markdown_output"
              ></span>
            </div>
          </div>
        </div>
        ...
        <div class="row mb-4">
          <div v-if="database.is_trial || database.is_new" class="col-auto">
            <span v-if="database.is_trial" class="badge text-bg-warning">{{
              t("components.database_list_row.is_trial")
            }}</span>
            <span v-if="database.is_new" class="badge text-bg-success">{{
              t("components.database_list_row.is_new")
            }}</span>
          </div>
        </div>
                ...
                <ul v-if="topic.sub_topics && topic.sub_topics.length">
                  <li v-for="sub_topic in topic.sub_topics" :key="sub_topic.id">
                    <LangLink
                      ...
                      :to="{
                        path: '/',
                        query: { topic: topic.id, sub_topics: sub_topic.id },
                      }"
                      >{{ sub_topic.name }}</LangLink
                    >
                  </li>
                </ul>
        ...
        <div
          v-if="database.media_types && database.media_types.length > 0"
          class="row"
        >
          <div class="col">
            <div>
              ...
              <ul class="list-inline">
                <li
                  ...
                  v-for="(media_type, index) in database.media_types"
                  :key="media_type.id"
                >
                  <LangLink
                    class="underlined"
                    :to="{ path: '/', query: { mediatype: media_type.id } }"
                    >{{ media_type.name }}</LangLink
                  ><span v-if="index + 1 < database.media_types.length"
                    >,
                  </span>
                </li>
              </ul>
        ...
        <div
          v-if="database.publishers && database.publishers.length > 0"
          class="row"
        >
          <div class="col">
            ...
            <ul class="list-inline">
              <li
                class="list-inline-item"
                v-for="(publisher, index) in database.publishers"
                :key="publisher.id"
              >
                {{ publisher.name
                }}<span v-if="index + 1 < database.publishers.length">, </span>
              </li>
            </ul>
          </div>
        </div>
            ...
            <div>
              ...
              <ul class="list-inline">
                ...
                >
                  ...
                  }}<span v-if="index + 1 < database.alternative_titles.length"
                    >,
                  </span>
                </li>
              </ul>
...
const {
  ...
  error,
  refresh,
} = await useFetch(`${"/api/database/"}${route.params.id}`, {
  params: payload,
});
...
function goBack() {
  display_back_url
    ? router.go(-1)
    : router.push({ path: "/", query: { lang: route.query.lang } });
}
</script>
