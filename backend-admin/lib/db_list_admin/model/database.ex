defmodule DbListAdmin.Model.Database do
 ...
 # import Access
  ...
  def remap(%{description: _, title: _} = database) do
    %{
      id: database.id,
      ...
      topics: database.topics |> Enum.map(fn item -> Model.Topic.remap(item) end),
      sub_topics: database.sub_topics |> Enum.map(fn item -> Model.SubTopic.remap(item) end),
      #terms_of_use: database.database_terms_of_use |> Enum.map(fn item -> Model.DatabaseTermsOfUse.remap(item, lang) end),
      media_types: database.media_types |> Enum.map(fn item -> Model.MediaType.remap(item) end),
