      ...
      <FormKit type="form" :actions="false">
        <div class="row">
          ...
          <div class="col-auto align-self-end">
            <router-link
              v-if="topic_initial_state.id"
              class="btn btn-light me-2"
              :to="{
                name: 'TopicShow',
                params: { id: topic_initial_state.id },
              }"
              >Cancel</router-link
            >
            <FormKit
        ...
        <div v-if="props.errors" class="row mt-3">
          <div class="col">
            <div
              v-if="props.errors.topic && props.errors.topic.length"
              class="alert alert-danger"
              role="alert"
            >
              <ul>
                <li v-for="(error, index) in props.errors.topic" :key="index">
                  {{ $t("topic.error_codes." + error.field) }}:
                  {{ $t("topic.error_codes." + error.error_code) }}
                </li>
              </ul>
            </div>
          </div>
        ...
        <div class="row">
          <div class="mb-3 col-6">
            <FormKit
              id="alternative_names_en"
              type="text"
              :classes="{
                input: 'form-control',
              }"
              label="Topic alternative names (en)"
              name="topic_initial_state.alternative_names_en"
              v-model="topic_initial_state.alternative_names_en"
              placeholder="Alternative names (comma separated list)"
            />
          </div>
          <div class="mb-3 col-6">
            <FormKit
              id="alternative_names_sv"
              type="text"
              :classes="{
                input: 'form-control',
              }"
        ...
        <div id="sub-topics-wrapper" v-if="topic_initial_state.sub_topics">
          ...
          <div v-if="props.errors && props.errors.sub_topics" class="row mt-3">
            <div class="col">
              <div
                v-if="props.errors.sub_topics"
                class="alert alert-danger"
                role="alert"
              >
                <ul>
                  <li
                    v-for="(error, index) in props.errors.sub_topics"
                    :key="index"
                  >
                    {{ $t("sub_topics.error_codes." + error.field) }}:
                    {{ $t("sub_topics.error_codes." + error.error_code) }}
                  </li>
                </ul>
              </div>
            </div>
          ...
          <ul
            v-if="topic_initial_state.sub_topics.length"
            id="sub-topics-list"
            class="list-unstyled"
          >
            <li
              :class="{ is_marked_for_removal: sub_topic.marked_for_removal }"
              v-for="(sub_topic, index) in topic_initial_state.sub_topics"
              :key="index"
            >
              <div class="row">
                ...
                <div class="col">
                  <button
                    @click.prevent="removeSubTopic(index)"
                    style="margin-top: 24px"
                    :disabled="sub_topic.marked_for_removal"
              ...
              <div class="row">
                <div class="col-5">
                  <FormKit
                    ...
                    label="Subtopic alternative names (en)"
                    :name="GUID() + '-en'"
                    v-model="sub_topic.alternative_names_en"
                    placeholder="Alternative names (comma separated list)"
                  />
                </div>
                <div class="col-5">
                  <FormKit
                    ...
                    label="Subtopic alternative names (sv)"
                    :name="GUID() + '-sv'"
                    v-model="sub_topic.alternative_names_sv"
                    placeholder="Alternative names (comma separated list)"
                  />
                </div>
              </div>
          ...
          <div class="row">
            <div class="col align-self-start">
              <button class="btn btn-light" @click.prevent="addSubTopic">
                New subtopic +
              </button>
            </div>
          </div>
...
export default {
  ...
  setup(props, ctx) {
    ...
    const isDirty = computed(() =>
      _.isEqual(topic, topic_initial_state.value) ? false : true
    );
    let isSaved = false;
    onMounted(() => {
      nextTick(() => {
    ...
    const saveTopic = () => {
      ...
      let sub_topics = [];
      if (
        topic_initial_state.value.sub_topics &&
        topic_initial_state.value.sub_topics.length
      ) {
