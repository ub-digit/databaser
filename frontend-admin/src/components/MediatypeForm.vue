      ...
      <FormKit type="form" :actions="false">
        <div class="row">
          ...
          <div class="col-auto align-self-end">
            <router-link
              v-if="mediatype_initial_state.id"
              class="btn btn-light me-2"
              :to="{
                name: 'MediatypeShow',
                params: { id: mediatype_initial_state.id },
              }"
              >Cancel</router-link
            >
            <FormKit
        ...
        <div v-if="props.errors" class="row mt-3">
          <div class="col">
            <div
              v-if="props.errors.media_type"
              class="alert alert-danger"
              role="alert"
            >
              <ul>
                <li
                  v-for="(error, index) in props.errors.media_type"
                  :key="index"
                >
                  {{ $t("mediatype.error_codes." + error.field) }}:
                  {{ $t("mediatype.error_codes." + error.error_code) }}
                </li>
              </ul>
            </div>
          </div>
        ...
        <div class="row">
          <div class="mb-3 col-6">
            <FormKit
              ...
              label="Mediatype alternative names (en)"
              name="mediatype_initial_state.alternative_names_en"
              v-model="mediatype_initial_state.alternative_names_en"
          ...
          <div class="mb-3 col-6">
            <FormKit
              ...
              label="Mediatype (sv)"
              name="mediatype_initial_state.alternative_names_sv"
              v-model="mediatype_initial_state.alternative_names_sv"
              placeholder="Alternative names (comma separated list)"
            />
          </div>
        </div>
...
export default {
  ...
  setup(props, ctx) {
    ...
    const isDirty = computed(() =>
      _.isEqual(mediatype, mediatype_initial_state.value) ? false : true
    );
    let isSaved = false;
