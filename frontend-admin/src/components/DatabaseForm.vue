<template>
  <div v-if="database_initial_state" class="edit-topic-wrapper">
    ...
    <FormKit type="form" :actions="false">
      <div class="row mb-4">
        ...
        <div class="col-auto align-self-end">
          <router-link
            v-if="database_initial_state.id"
            class="btn btn-light me-2"
            :to="{
              name: 'DatabaseShow',
              params: { id: database_initial_state.id },
            }"
            >Cancel</router-link
          >
          <router-link
        ...
        <div class="col-auto">
          <div class="form-check">
            <input
              ...
              type="checkbox"
              id="published"
              v-model="database_initial_state.published"
            />
        ...
        <div class="col-auto">
          <div class="form-check">
            <input
              ...
              type="checkbox"
              id="isPopular"
              v-model="database_initial_state.is_popular"
            />
            <label class="form-check-label" for="isPopular"
              >Promoted as popular</label
            >
          </div>
        </div>
        <div class="col-auto">
          <div class="form-check">
            <input
              ...
              type="checkbox"
              id="isNew"
              v-model="database_initial_state.is_new"
            />
        ...
        <div class="col-auto">
          <div class="form-check">
            <input
              ...
              type="checkbox"
              id="isTrial"
              v-model="database_initial_state.is_trial"
            />
      ...
      <div class="row mb-4">
        <div class="col-12">
          ...
          <ul
            class="list-unstyled"
            v-if="database_initial_state.alternative_titles"
          >
            <li
              class="mb-4"
              v-for="(
                alternative_title, index
              ) in database_initial_state.alternative_titles"
              :key="index"
            >
              <div class="row">
                <div class="col-4">
                ...
                <div class="col-2 d-flex align-items-center">
                  <div class="form-check mt-4">
                    <input
                      class="form-check-input"
                      type="checkbox"
                      :name="'is_hidden' + index"
                      :id="'is_hidden' + index"
                      help="Hide in interface"
                      v-model="alternative_title.is_hidden"
                    />
                    <label class="form-check-label" :for="'is_hidden' + index"
                      >Hide</label
                    >
                  </div>
                </div>
                <div class="col-2">
                  <button
                    @click.prevent="removeAltTitle(index)"
                    class="mt-4 btn btn-danger float-end"
                  >
                    Remove
                  </button>
                </div>
          ...
          <button @click.prevent="addAltTitle()" class="btn btn-primary">
      ...
      <div class="row">
        <div class="mb-3 col-6">
          <label for="desc-preview-en" class="formkit-label"
            >Description preview (en)</label
          >
          <div
            id="desc-preview-en"
            class="faded"
            v-html="desc_en_markdown_output"
          ></div>
        </div>

        <div class="mb-3 col-6">
          <label for="desc-preview-sv" class="formkit-label"
            >Description preview (sv)</label
          >
          <div
            id="desc-preview-sv"
            class="faded"
            v-html="desc_sv_markdown_output"
          ></div>
        </div>
      </div>
      <!-- end row -->
      <div class="row">
        <div class="mb-3 col-6">
          ...
          <ul
            ...
            class="list-unstyled"
            v-if="database_initial_state.topics"
          >
            <li v-for="topic in database_initial_state.topics" :key="topic.id">
              <div class="form-check">
                <input
                  :disabled="subTopicSelected(topic)"
                  class="form-check-input"
                  type="checkbox"
                  :id="topic.id"
                ...
                <label class="form-check-label" :for="topic.id"
                  >{{ topic.name_en }} / {{ topic.name_sv }}</label
                >
              </div>
                ...
                <li v-for="subtopic in topic.sub_topics" :key="subtopic.id">
                  <div class="form-check">
                    <input
                      ...
                      type="checkbox"
                      :id="subtopic.id"
                      v-model="subtopic.selected"
                    />
                    <label class="form-check-label" :for="subtopic.id"
                      >{{ subtopic.name_sv }} / {{ subtopic.name_en }}</label
                    >
                  </div>
                </li>
              </ul>
              ...
              <div v-if="topic">
                <div v-if="topic.selected" class="form-check">
                  <input
                    ...
                    type="checkbox"
                    :id="'r_' + topic.id"
                    v-model="topic.recommended"
                  />
                  <label class="form-check-label" :for="'r_' + topic.id"
                    >{{ topic.name_en }} / {{ topic.name_sv }}</label
                  >
                </div>
                <ul class="" v-if="topic.sub_topics.length">
                  <div v-for="subtopic in topic.sub_topics" :key="subtopic.id">
                    <li v-if="subtopic.selected">
                      <div class="form-check">
                        <input
                          :disabled="!subtopic.selected"
                          class="form-check-input"
                          type="checkbox"
                          :id="'r_' + subtopic.id"
                          v-model="subtopic.recommended"
                        />
                        <label
                          class="form-check-label"
                          :for="'r_' + subtopic.id"
                          >{{ subtopic.name_sv }} /
                          {{ subtopic.name_en }}</label
                        >
                      </div>
                    </li>
                  </div>
      ...
      <div class="row mb-4">
        <div class="col">
          ...
          <VueMultiselect
            track-by="id"
            v-model="publishers"
            @select="addSelectedPublisher"
            @remove="removeSelectedPublisher"
            :multiple="true"
            label="name"
      ...
      <div class="row">
        <div class="col">
          ...
          <ul
            class="list-unstyled"
            v-if="
              database_initial_state.media_types &&
              database_initial_state.media_types.length
            "
          >
            <li
              v-for="mediatype in database_initial_state.media_types"
              :key="mediatype.id"
            >
              <div class="form-check">
                <input
                  ...
                  v-model="mediatype.selected"
                />
                <label
                  :for="'mediatype_' + mediatype.id"
                  class="form-check-label"
                  >{{ mediatype.name_en }} / {{ mediatype.name_sv }}</label
                >
              </div>
            </li>
          </ul>
      ...
      <div class="row mb-4">
        <div class="col-12">
            ...
            <div class="col">
              <div class="form-check">
                <input
                  ...
                  class="form-check-input"
                  id="hide-direct-link"
                  v-model="database_initial_state.direct_link_is_hidden"
                />
                <label for="hide-direct-link" class="form-check-label"
                  >Hide direct link in search results</label
                >
              </div>
            </div>
          </div>
          ...
          <ul class="list-unstyled" v-if="database_initial_state.urls">
            <li
              class="mb-4"
              v-for="(url, index) in database_initial_state.urls"
              :key="index"
            >
              <div class="row">
                <div class="col-5">
          ...
          <button @click.prevent="addDatabaseUrl()" class="btn btn-primary">
            Add another url +
          </button>
        </div>
      </div>
      ...
      <div
        v-if="
          database_initial_state.terms_of_use &&
          database_initial_state.terms_of_use.length
        "
        class="terms_of_use"
      >
        <TermsOfUse
          ...
          :terms_of_use="database_initial_state.terms_of_use"
        />
      </div>
    </FormKit>
    <!-- end form -->
    ...
    <div class="row">
      <div class="col mb-3">
        ...
        <AccessInformationCode
          @updateAccessInformationCode="updateAccessInformationCode"
          :access_information_code="
            database_initial_state.access_information_code
          "
        />
      </div>
    </div>
        ...
        <div class="col-auto mb-3">
          <div class="form-check">
            <input
              class="form-check-input"
              type="checkbox"
              id="malfunction_message_active"
              v-model="database_initial_state.malfunction_message_active"
            />
            <label class="form-check-label" for="malfunction_message_active"
              >Enables the malfunction message (ie makes it visible to
              users)</label
            >
          </div>
        </div>
    ...
    <div class="row">
      <div class="mb-3 col-6">
        <label for="malfunction_message_en" class="formkit-label"
          >Malfunction message preview (en)</label
        >
        <div
          id="malfunction_message_en"
          class="faded"
          v-html="malfunction_message_en_output_en"
        ></div>
      </div>
      <div class="mb-3 col-6">
        <label for="malfunction_message_sv" class="formkit-label"
          >Malfunction message preview (sv)</label
        >
        <div
          id="malfunction_message_sv"
          class="faded"
          v-html="malfunction_message_en_output_sv"
        ></div>
      </div>
    </div>
    <div class="row">
...
export default {
  ...
  setup(props, ctx) {
    ...
    const desc_sv_markdown_output = computed(() =>
      marked(database_initial_state.value.description_sv)
    );
    const malfunction_message_en_output_en = computed(() =>
      database_initial_state.value.malfunction_message_en
        ? marked(database_initial_state.value.malfunction_message_en)
        : ""
    );
    const malfunction_message_en_output_sv = computed(() =>
      database_initial_state.value.malfunction_message_sv
        ? marked(database_initial_state.value.malfunction_message_sv)
        : ""
    );
    const isDirty = computed(() =>
      _.isEqual(database, database_initial_state.value) ? false : true
    );
    let isSaved = false;
    watch(
    ...
    const saveDatabase = () => {
      const val = database_initial_state.value.urls.filter(
        (item) => item.url != ""
      );
    ...
    const addAltTitle = (val) => {
      database_initial_state.value.alternative_titles.push({
        title_en: "",
        title_sv: "",
      });
    };
    ...
    const updateTermOfUse = (code, val) => {
      let term_of_use = database_initial_state.value.terms_of_use.find(
        (term_of_use) => code === term_of_use.code
      );
    ...
    const addSelectedPublisher = (selectedOption, id) => {
      const found = database_initial_state.value.publishers.find(
        (publisher) => selectedOption.id === publisher.id
      );
      found.selected = true;
      console.log(found);
    };
    ...
    const addAllSelectedPublishersOnLoad = () => {
      const found = database_initial_state.value.publishers.filter(
        (publisher) => {
