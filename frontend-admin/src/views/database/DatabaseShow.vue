  ...
  <div v-else>
    <div v-if="database && database.id" class="databaseshow-wrapper">
        ...
        <div class="col">
          <div class="float-end">
            <router-link
              ...
              :to="{ name: 'DatabaseEdit', params: { id: database.id } }"
              >Edit</router-link
            >
            <a
              href="#"
              @click.prevent="removeDatabase(database)"
      ...
      <div class="row pb-4">
        <div class="col-auto">
          Published:
          <span v-if="database.published" class="text-success">Yes</span
          ><span class="text-danger" v-else>No</span>
        </div>
        <div class="col-auto">
          Promoted as popular:
          <span v-if="database.is_popular" class="text-success">Yes</span
          ><span class="text-danger" v-else>No</span>
        </div>
        <div class="col-auto">
          Marked as new:
          <span v-if="database.is_new" class="text-success">Yes</span
          ><span class="text-danger" v-else>No</span>
        </div>
        <div class="col-auto">
          Marked as trial:
          <span v-if="database.is_trial" class="text-success">Yes</span
          ><span class="text-danger" v-else>No</span>
        </div>
      </div>
      ...
      <div class="row pb-4">
        <div class="col">
          <label class="db-property-header fw-bold" for="desc_en"
      ...
      <div class="row pb-4">
        ...
        <div class="col-6">
          ...
          <ul class="list-unstyled" v-if="database.topics">
            <li v-for="topic in database.topics" :key="topic.id">
              <div v-if="topic.recommended">
                {{ topic.name_en }} / {{ topic.name_sv }}
              </div>
              <ul v-if="topic.sub_topics.length">
                <div v-for="subtopic in topic.sub_topics" :key="subtopic.id">
                  <li v-if="subtopic.recommended">
                    <div v-if="subtopic.recommended">
                      {{ subtopic.name_en }} / {{ subtopic.name_sv }}
                    </div>
                  </li>
                </div>
      ...
      <div class="row pb-4">
        <div class="col">
          ...
          <div class="row mb-4">
            <div class="col">
              <div class="text-danger" v-if="database.direct_link_is_hidden">
                Direct link is hidden in search result list
              </div>
              <div class="text-success" v-else>
                Direct link is visible in search result list
              </div>
            </div>
          </div>
          <ul class="list-unstyled" v-if="database.urls">
            <div v-for="(url, index) in database.urls" :key="index">
              <li>
                {{
                  url.title && url.title.length ? url.title : "Title is missing"
                }}
                / <a target="_new" :href="url.url">{{ url.url }}</a>
              </li>
            </div>
          </ul>
        </div>
          ...
          <ul class="list-unstyled" v-if="database.terms_of_use">
            <li v-for="term_of_use in database.terms_of_use">
              ...
              <ul>
                <li
                  v-if="
                    term_of_use.description_en.length ||
                    term_of_use.description_sv.length
                  "
                >
                  <div>
      ...
      <div class="row pb-4">
        <div class="col">
          ...
          <div
            ...
            v-if="database.malfunction_message_active"
          >
            Enabled
          </div>
...
export default {
  ...
  setup(props, ctx) {
    ...
    const desc_en_markdown_output = computed(() =>
      marked(database.value.description_en)
    );
    const desc_sv_markdown_output = computed(() =>
      marked(database.value.description_sv)
    );
    const malfunction_message_en_output_en = computed(() =>
      database.value.malfunction_message_en
        ? marked(database.value.malfunction_message_en)
        : ""
    );
    const malfunction_message_en_output_sv = computed(() =>
      database.value.malfunction_message_sv
        ? marked(database.value.malfunction_message_sv)
        : ""
    );
